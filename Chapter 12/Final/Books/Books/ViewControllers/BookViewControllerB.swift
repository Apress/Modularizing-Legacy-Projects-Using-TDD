...
class BookViewControllerB: UIViewController {

    public var book:BookModel? = nil
    ...
    override func viewDidLoad() {
        ...
        self.bookViewPresenter?.delegate = self

        self.title = self.book?.title
        self.desc?.text = self.book?.bookDescription
        self.date?.text = self.book?.createdDate
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Buy", style: .plain, target: self, action: #selector(showBuyModal))

        self.bookViewPresenter?.fetchBookReviews(title: (self.book?.title)!)
    ...
    @IBAction func showBuyModal() {
        // add to favorites
        let alertController = UIAlertController(title: "Buy links", message:nil, preferredStyle: .actionSheet)
        alertController.addAction(UIAlertAction(title: "Amazon", style: .default, handler: { action in
            self.buyByAmazon()
        }))
        
        alertController.addAction(UIAlertAction(title: "Apple books", style: .default, handler: { action in
            self.buyByAppleBooks()
        }))
        
        alertController.addAction(UIAlertAction(title: "Barnes and Nobel", style: .default, handler: { action in
            self.buyByBarnesAndNoble()
        }))
        
        alertController.addAction(UIAlertAction(title: "Books a million", style: .default, handler: { action in
            self.buyByBooksAMillion()
        }))
        
        alertController.addAction(UIAlertAction(title: "Book shop", style: .default, handler: { action in
            self.buyByBookshop()
        }))
        
        alertController.addAction(UIAlertAction(title: "Indie bound", style: .default, handler: { action in
            self.buyByIndieBound()
        }))
...
extension BookViewControllerB: BookViewPresenterDelegate {
    func reviewDidFinish(_ review: String?) {
        guard let review = review else {
            return
        }
        
        self.bookDetails.append([reviewKey: review])
        self.tableView?.reloadData()
    }
}

extension BookViewControllerB :UITableViewDelegate {
...
extension BookViewControllerB :UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let item = self.bookDetails[indexPath.row]
        if item.keys.first == reviewKey {
            cell.value?.accessibilityIdentifier = "book_review"
        } else {
            cell.value?.accessibilityIdentifier = nil
        }
